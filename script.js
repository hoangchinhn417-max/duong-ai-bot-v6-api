...
let API_BASE=(window.VYRO_API_BASE!==undefined?window.VYRO_API_BASE:localStorage.getItem('VYRO_API_BASE'))||'',eventSource=null,lastDataTime=0,heartbeatTimer=null;

function $(id){return document.getElementById(id)}

async function syncUsersFromServer(){
  try{
    let r=await fetch(API_BASE+'/api/users',{cache:'no-store'});
    if(!r.ok) return;
    let arr=await r.json();
    if(Array.isArray(arr)&&arr.length){users=arr;localStorage.setItem(USER_KEY,JSON.stringify(arr));renderUserTable&&renderUserTable();}
  }catch(e){}
}
async function syncUsersToServer(){
  try{await fetch(API_BASE+'/api/users',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(users)});}catch(e){}
}
function initUsers(){
  syncUsersFromServer();
...
function saveUsers(){localStorage.setItem(USER_KEY,JSON.stringify(users));syncUsersToServer()}
...
async function loginNow(){
  ...
  try{
    let r=await fetch(API_BASE+'/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});
    if(r.ok){
      let data=await r.json();
      currentUser=data.user;
      let local=users.find(x=>x.username===u); if(local){currentUser={...local,...currentUser}}
      localStorage.setItem(SESSION_KEY,u);
      enterApp(currentUser); setLoginLoading(false); return;
    }
  }catch(e){}
  setTimeout(()=>{
...
function connectApi(){
  try{
    ...
    eventSource.onopen=()=>setApiState(true);
    eventSource.onerror=()=>setApiState(false);
    eventSource.onmessage=e=>{try{let d=JSON.parse(e.data);if(d.type==='signal')updateSignal(d.payload)}catch(err){}}
  }catch(e){setApiState(false)}
}
function setApiState(ok){$('apiStatusText').innerText=ok?'ONLINE':'STANDBY';$('connectionText').innerText=ok?'ONLINE':'STANDBY'}
...
function smcObj(s){return s.smc||s.liquidityZones||s.liquidity_zones||s.zones||{};}
function fmtPrice(v){
  if(v===undefined||v===null||v==='') return '--';
  let n=Number(v);
  if(isNaN(n)) return String(v);
  return n.toFixed(n>=1000?2:3).replace(/\.0+$/,'');
}
function fmtRange(a,b){
  if(a===undefined || b===undefined || a===null || b===null || a==='' || b==='') return '--';
  let x=Number(a), y=Number(b);
  if(!isNaN(x) && !isNaN(y) && x>y){let t=x;x=y;y=t;}
  return fmtPrice(x)+' - '+fmtPrice(y);
}
function zoneWidth(s){
  let z=pick(smcObj(s),['zoneWidth','zone_width','atrZone','atr_zone']);
  if(z!==undefined && !isNaN(Number(z)) && Number(z)>0) return Number(z);
  let atr=Number(pick(s,['atr','ATR']));
  if(!isNaN(atr) && atr>0) return Math.max(atr*0.20,0.5);
  return 10;
}
function zoneFromCenter(center,width){
  let c=Number(center); if(isNaN(c) || c<=0) return [undefined,undefined];
  let w=Number(width||10); return [c-w,c+w];
}
function updateZones(s){
  const z=smcObj(s), w=zoneWidth(s);
  let sellLow=pick(z,['sellLow','sell_low','sellZoneLow','sell_zone_low','bslLow','bsl_low']);
  let sellHigh=pick(z,['sellHigh','sell_high','sellZoneHigh','sell_zone_high','bslHigh','bsl_high']);
  let buyLow=pick(z,['buyLow','buy_low','buyZoneLow','buy_zone_low','sslLow','ssl_low']);
  let buyHigh=pick(z,['buyHigh','buy_high','buyZoneHigh','buy_zone_high','sslHigh','ssl_high']);
  let ntLow=pick(z,['noTradeLow','no_trade_low','midLow','mid_low','inducementLow','inducement_low']);
  let ntHigh=pick(z,['noTradeHigh','no_trade_high','midHigh','mid_high','inducementHigh','inducement_high']);
  const bsl=pick(z,['bsl','BSL','bslLine','bsl_line','buySideLiquidity','buy_side_liquidity']);
  const ssl=pick(z,['ssl','SSL','sslLine','ssl_line','sellSideLiquidity','sell_side_liquidity']);
  const mid=pick(z,['mid','middle','inducement','inducementLine','inducement_line']);
  if((sellLow===undefined||sellHigh===undefined) && bsl!==undefined){[sellLow,sellHigh]=zoneFromCenter(bsl,w)}
  if((buyLow===undefined||buyHigh===undefined) && ssl!==undefined){[buyLow,buyHigh]=zoneFromCenter(ssl,w)}
  if((ntLow===undefined||ntHigh===undefined) && mid!==undefined){[ntLow,ntHigh]=zoneFromCenter(mid,w*0.5)}
  if($('sellZone')) $('sellZone').innerText=fmtRange(sellLow,sellHigh);
  if($('buyZone')) $('buyZone').innerText=fmtRange(buyLow,buyHigh);
  if($('noTradeZone')) $('noTradeZone').innerText=fmtRange(ntLow,ntHigh);
  if($('sellZoneNote')) $('sellZoneNote').innerText=bsl!==undefined?'BSL / Buy-side liquidity từ bot SMC':'Chờ BSL từ bot SMC';
  if($('buyZoneNote')) $('buyZoneNote').innerText=ssl!==undefined?'SSL / Sell-side liquidity từ bot SMC':'Chờ SSL từ bot SMC';
  if($('noTradeNote')) $('noTradeNote').innerText=mid!==undefined?'Inducement / Mid range từ bot SMC':'Chờ Inducement từ bot SMC';
}
function clearPageLocks(){
  document.body.classList.remove('locked','modal-open','loading','disabled');
...
function updateSignal(s){
  ...
  $('deltaSub').innerText=nval(dv)>0?'Buyer attack':nval(dv)<0?'Seller attack':'Neutral attack';$('power').innerText=pv;$('buySell').innerText=bs;$('conf').innerText=sc+'%';$('lastUpdate').innerText=fmt(s.receivedAt||new Date().toISOString());
  updateZones(s);
  $('trend').innerText=s.trend||'N/A';$('liquidity').innerText=s.liquidity||'N/A';$('pressure').innerText=s.pressure||sig;$('risk').innerText=s.risk||'Medium';$('action').innerText=s.action||'Follow setup';$('source').innerText=s.source||'MT5';$('dominance').innerText=dom;$('dominanceNote').innerText=`Flow ${fv} · Delta ${dv} · Power ${pv}`;$('lastTick').innerText=p;lastDataTime=Date.now();addHistory(sig,p,fv,dv)
}
