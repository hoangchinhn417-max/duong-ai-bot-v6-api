...
function demoSignal(){
  updateSignal({symbol:'XAUUSD',timeframe:'M1',signal:'WAIT',score:55,price:'--',rsi:'--',flow:'--',delta:'--',power:'--',buySell:'--',reason:'API chưa có dữ liệu mới. Dashboard vẫn sẵn sàng nhận tín hiệu MT5.',trend:'Neutral',liquidity:'Waiting',pressure:'Waiting',risk:'Medium',source:'DEMO',sellZone:'--',buyZone:'--'});
}
...
function zoneText(v){
  ...
  if(typeof v==='object'){
    let top=v.top||v.high||v.from||v.upper||v.price1;
    let bot=v.bottom||v.low||v.to||v.lower||v.price2;
    if(top||bot) return `${top||'--'} - ${bot||'--'}`;
    return JSON.stringify(v);
  }
  return String(v);
}
function getSellZone(s){
  return s.sellZone || s.sell_zone || s.supply || s.supplyZone || s.supply_zone || s.smcSellZone || s.raw?.sellZone || s.raw?.supply || s.raw?.SUPPLY || s.raw?.supply_line_label || '--';
}
function getBuyZone(s){
  return s.buyZone || s.buy_zone || s.demand || s.demandZone || s.demand_zone || s.smcBuyZone || s.raw?.buyZone || s.raw?.demand || s.raw?.DEMAND || s.raw?.demand_line_label || '--';
}
function getNoTradeZone(s){
  return s.noTradeZone || s.no_trade_zone || s.chopZone || s.midZone || '--';
}
...
function updateSignal(s){
  ...
  $('trend').innerText=s.trend||'N/A';$('liquidity').innerText=s.liquidity||'N/A';$('pressure').innerText=s.pressure||sig;$('risk').innerText=s.risk||'Medium';$('action').innerText=s.action||'Follow setup';$('source').innerText=s.source||'MT5';$('dominance').innerText=dom;$('dominanceNote').innerText=`Flow ${fv} · Delta ${dv} · Power ${pv}`; if($('sellZone')) $('sellZone').innerText=zoneText(getSellZone(s)); if($('buyZone')) $('buyZone').innerText=zoneText(getBuyZone(s)); if($('noTradeZone')) $('noTradeZone').innerText=zoneText(getNoTradeZone(s)); if($('sellZoneNote')) $('sellZoneNote').innerText=s.raw?.supply_detail||s.supplyDetail||'SMC Supply từ bot'; if($('buyZoneNote')) $('buyZoneNote').innerText=s.raw?.demand_detail||s.demandDetail||'SMC Demand từ bot'; $('lastTick').innerText=p;lastDataTime=Date.now();addHistory(sig,p,fv,dv)
}
